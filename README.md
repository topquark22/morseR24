...
An Arduino sketch that converts text to Morse code and other formats, outputs to GPIO pin, and can transmit to slave devices using an nRF24L01 radio.
...
Here, an LM555 timer in astable mode generates a tone that is sent to a mono audio jack. The pin D5 output signal is sent to pin 4 (active low reset) of the LM555 to modulate the signal. This example uses a Nano V3 (compatible) with external radio connected to the SPI bus. This circuit can be used in master mode (with or without radio) or slave mode.
